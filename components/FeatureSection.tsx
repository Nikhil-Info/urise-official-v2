...
export default function FeatureSection() {
  ...
  return (
    <section
      ref={ref}
      className="relative isolate overflow-hidden bg-[#f5fbf8] py-28"
    >
      <motion.div
        style={{ y: blobY1 }}
        className="pointer-events-none absolute -top-20 -left-32 h-80 w-80 rounded-full bg-emerald-200/50 blur-3xl"
      />
      <motion.div
        style={{ y: blobY2 }}
        className="pointer-events-none absolute bottom-0 -right-32 h-96 w-96 rounded-full bg-teal-200/40 blur-3xl"
      />
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        >
          <div className="mb-5 inline-flex items-center gap-2 rounded-full bg-white/80 px-3.5 py-1.5 text-xs font-medium text-[#062018] ring-1 ring-emerald-900/[0.06] backdrop-blur">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
            Why founders pick us
          </div>
          <h2 className="text-balance text-4xl font-semibold tracking-[-0.02em] text-[#062018] sm:text-5xl">
            Built for the moment you go live.
          </h2>
          <p className="mt-4 text-balance text-[#4b6b5c]">
...
function FeatureCard({
  ...
  return (
    <motion.div
      ...
      className="group relative overflow-hidden rounded-2xl bg-white p-6 ring-1 ring-emerald-900/[0.05] shadow-[0_10px_30px_-20px_rgba(4,60,40,0.22)] transition hover:-translate-y-1 hover:shadow-[0_20px_40px_-20px_rgba(4,60,40,0.28)]"
    >
      <div
        className={`mb-5 inline-grid h-11 w-11 place-items-center rounded-xl bg-gradient-to-br ${feature.accent} ring-1 ring-emerald-900/[0.05]`}
      >
        <Icon className={`h-5 w-5 ${feature.iconColor}`} strokeWidth={2.2} />
      </div>
      <h3 className="text-lg font-semibold tracking-tight text-[#062018]">
        {feature.title}
      </h3>
      <p className="mt-2 text-sm leading-relaxed text-[#4b6b5c]">{feature.desc}</p>
    </motion.div>
  );
}
